...
class Board
  ...
  #Display the board with empty slots 
  def display
  ...
  #Find an element at cortain index in the @cells array
  def position(index)
    @cells[index.to_i-1] 
  end
  #Check if the entier @cells array ocupid with ether "X" or "O"
  def full?
  ...
  #Check if a cortain position(at index number) is ocupied with either "X" or "O"  
  def taken?(index)
