module Players
  class Computer < Player
    ...
    def move(a)
      if @game.board.turn_count == 0
      ...
      elsif @game.board.turn_count == 1 #if second player, first move: check if middle is occupied; if yes, go to corner cells.
        if !@game.board.taken?("5")
          a = "5"
        else
          a = ["1","3","7","9"][rand(0..3)]
        end
      elsif @game.board.turn_count == 2 #second move if AI went first (go to diagonal corner cell)
        a = ["1","3","7","9"].detect{|a|!@game.board.taken?(a)} #occupy the corners whichever one was not occupied
      elsif @game.board.turn_count == 3
        ...
        if (@game.board.position(1) == @game.board.position(9) || @game.board.position(3) == @game.board.position(7)) #if opponent occupied corners
          a = "5"
        elsif @game.board.position.taken?("5") #check if the existing 2 moves by opponents is in the winning combo
