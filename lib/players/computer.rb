module Players
  ...
  class Computer < Player
    ...
    def move(board)
      # Remember -- for Game#turn to work, #move has to return a string that is one higher than
      # the index number of the cell it wants to take.
      position = nil
        # Look for center square first and take it if open
      if board.cells[4] == " "
        position = 5
        puts "#{token} chose #{position} because it likes the middle."
          # If center square is not open, go through each of the winning combos and see if its own
          # token has 2 out of the three spots and if and that the third spot is open. Take the
          # open square if so to win.
        ...
        puts "#{token} chose #{position} to win."
          # If computer cannot win,  go through each of the winning combos and see if its opponent
          # has 2 out of the three spots and if and that the third spot is open. Take the open square
          # if so to block opponent.
