class Player::Computer < Player
  ...
  #The #complete_combo method return one single win combinatios with 2 cells with equal tokens and one cell not  taken

  def complete_combo?(board, token)
    Game::WIN_COMBINATIONS.detect do |combo|
      (
        (board.cells[combo[0]] == token && board.cells[combo[1]] == token) && !board.token?(combo[2]+1)
      ) ||
      (
        (board.cells[combo[1]] == token && board.cells[combo[2]] == token) && !board.token?(combo[0]+1)
      ) ||
      (
  ...
  def block(board)
    ...
    if blocking_combo && complete_combo?(board, self.opponent_token).count{|index| board.position(index+1) == self.opponent_token} == 2
