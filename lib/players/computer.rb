  ...
  class Computer < Player
    def move(board)
      ...
      Game::WIN_COMBINATIONS.detect do |combo|
        #Search for winning combination first by checking if 2 out of 3 winning cells are filled
        #and 3rd cell is empty
        if board.cells[combo[0]] == self.token && board.cells[combo[1]] == self.token && !board.taken?(combo[2]+1)
          ...
          next_move = combo[0]+1
        #If no winning combination, check to see if opponent has 2 out of 3 winning cells filled
        #then block 3rd cell.
      ...
      #If neither player can win, make a move based on heirarchy: center first, then corners, then rest
      if !next_move
        if !board.taken?(5)
          next_move = "5"
        elsif !board.taken?(1)
