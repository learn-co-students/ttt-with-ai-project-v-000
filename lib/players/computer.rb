module Players
    ...
    # winnable 
    # Returns a hash. If the game is winnable on this move, return { truth: true, move: N }, where N is the winning move. Otherwise return { truth: false, move: nil }
    ...
    # The computer tries to (1) win, (2) block the opponent's imminent win, (3) move into the middle, (4) move into an empty corner, (5) move into an empty space.
      ...
      if win_hash[:truth]
      	...
      	if block_hash[:truth]
      		block_hash[:move]
      	elsif !board.taken?(Board::MIDDLE)
      		Board::MIDDLE.to_s
        else
      		corners = Board::CORNERS.select {|index| board.cells[index] == " "}
      		if !corners.empty?
      			(corners[0] + 1).to_s
      		else
